...
class Forecast:
    forecast = {}
    hourForecast = {}
    icons={"Mostly Cloudy":"mostly_cloudy",
    ...
    def __init__(self,url = "https://weather.com/pl-PL/weather/tenday/l/93409ef628e2eccc8fe84493beb24470c722d12ef4632a9c49250af345ba81ef"):
        try:
            ...
            for div in filteredDivs:
                ...
                self.hourForecast[hour]['temperature'] = div.find_all(attrs={"data-testid": "TemperatureValue"})[0].text
                self.hourForecast[hour]['wind'] = div.find_all(attrs={"data-testid": "Wind"})[0].text.replace(u'\xa0', u' ')
                self.hourForecast[hour]['rain'] = div.find_all(attrs={"data-testid": "PercentageValue"})[0].text.replace(u'\xa0', u' ')
                self.hourForecast[hour]['icon'] = div.find_all(attrs={"data-testid": "Icon"})[0].text
    ...
    def DrawForecast(self,draw:ImageDraw.ImageDraw,fonts,icons,x1:int,y1:int,x2:int,y2:int):
        position = 0
        skipFirst = True

        if len(self.forecast) == 0: raise Exception("No loaded forecast")
        ...
        for forecastDay in self.forecast:
            ...
            if skipFirst != True and position <= 6:
                ...
                #draw weather icon    
                iconName = self.icons[self.forecast[forecastDay]['Dzień']['icon']] + "_small" 
                iconWidth = icons[iconName].width       
                draw._image.paste(icons[iconName],(leftBorder + 5 + round((70 - iconWidth)/2), y1+25))
    ...
    def DrawHourly(self,draw:ImageDraw.ImageDraw,fonts,icons,x1:int,y1:int,x2:int,y2:int):
        position = 0
        if len(self.forecast) == 0: raise Exception("No loaded forecast")
        ...
        for forecastHour in self.hourForecast:
            ...
            if position <= 8:
                ...
                #draw hour title
                dayTextLen = draw.textlength(forecastHour, font=fonts['font14'], direction=None, features=None)
                dayTextPos = round(leftBorder + boxWidth / 2 - dayTextLen / 2)
                draw.text((dayTextPos, y1+5),forecastHour,0,font=fonts['font14'])
                
                #draw weather icon    
                iconName = self.icons[self.hourForecast[forecastHour]['icon']] + '_small'
                iconWidth = icons[iconName].width       
                draw._image.paste(icons[iconName],(leftBorder + round((70 - iconWidth)/2), y1+25))
                ...
                #draw wind
                wind = re.findall(r'\d+', self.hourForecast[forecastHour]["wind"])[0] + ' km/h'
                windTextLen = draw.textlength(wind, font=fonts['font14light'], direction=None, features=None)
                windTextPos = round(leftBorder + boxWidth / 2 - windTextLen / 2)
                draw.text((windTextPos, y1+78),wind,0,font=fonts['font14light'])
                
                #draw wind icon
                if int(re.findall(r'\d+', self.hourForecast[forecastHour]["wind"])[0]) > 15:
                    iconName = 'wind_small'
                    iconWidth = icons[iconName].width       
                    draw._image.paste(icons[iconName],(leftBorder + round((70 - iconWidth)/2), y1+96))

                #draw rain
                rain = self.hourForecast[forecastHour]["rain"]
                tempTextLen = draw.textlength(rain, font=fonts['font18'], direction=None, features=None)
                tempTextPos = round(leftBorder + boxWidth / 2 - tempTextLen / 2)
                draw.text((tempTextPos+1, y1+126),rain,0,font=fonts['font14'])
                
                #draw rain icon
                if int(re.findall(r'\d+', self.hourForecast[forecastHour]["rain"])[0]) > 25:
                    iconName = 'rain_percent_small'
                    iconWidth = icons[iconName].width       
                    draw._image.paste(icons[iconName],(leftBorder + round((70 - iconWidth)/2-6), y1+147))
