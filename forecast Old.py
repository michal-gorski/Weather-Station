    ...
    def __init__(
        ...
        try:
            ...
            for div in filteredDivs:
                day = div.h2.text.strip()
                
                if day not in self.forecast:
                    self.forecast[day] = dict()
                
                self.forecast[day]["temperature"] = div.find_all(
                    attrs={"data-testid": "TemperatureValue"}
                )[0].text
                self.forecast[day]["wind"] = div.find_all(
                    attrs={"data-testid": "Wind"}
                )[0].text.replace("\xa0", " ")
                try:
                    self.forecast[day]["rain"] = div.find_all(
                        attrs={"data-testid": "PercentageValue"}
                    )[0].text.replace("\xa0", " ")
                except:
                    self.forecast[day]["rain"] = '0'

                self.forecast[day]["icon"] = div.find_all(
